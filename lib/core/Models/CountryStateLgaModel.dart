...
class CountryStateLgaModel {
  CountryStateLgaModel({
    required this.state,
  });

  final LocationState? state;

  CountryStateLgaModel copyWith({
    LocationState? state,
  }) {
    return CountryStateLgaModel(
      state: state ?? this.state,
    );
  }

  factory CountryStateLgaModel.fromJson(Map<String, dynamic> json) {
    return CountryStateLgaModel(
      state: json["state"] == null ? null : LocationState.fromJson(json["state"]),
    );
  }
  ...
  bool operator ==(Object other) {
    ...
    return other is CountryStateLgaModel && other.state?.id == state?.id;
  }

  @override
  int get hashCode => state?.id.hashCode ?? 0;
}
...
class LocationState {
  ...
  factory LocationState.fromJson(Map<String, dynamic> json) {
    return LocationState(
      name: json["name"],
      id: json["id"],
      locals: json["locals"] == null ? [] : List<Local>.from(json["locals"]!.map((x) => Local.fromJson(x))),
    );
  }
  ...
  Map<String, dynamic> toJson() => {
        "name": name,
        "id": id,
        "locals": locals.map((x) => x.toJson()).toList(),
      };

  @override
  String toString() {
    return "$name, $id, $locals, ";
  }

  @override
  bool operator ==(Object other) {
    if (identical(this, other)) return true;
    return other is LocationState && other.id == id;
  }
...
List<CountryStateLgaModel> getCountryLocation() {
  return List<CountryStateLgaModel>.from(nigerianLocation.map((x) => CountryStateLgaModel.fromJson(x)));
}

List<Map<String, dynamic>> nigerianLocation = [
  {
    "state": {
      "name": "Abia",
      "id": 1,
