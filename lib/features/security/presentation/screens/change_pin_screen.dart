...
class _ChangePinScreenState extends State<ChangePinScreen> {
  ...
  Future<void> _initiate() async {
    ...
    final res = await _pinApi
        .initiateChange(const InitiatePinRequest(channel: ApiConfig.defaultOtpChannel));
    if (!mounted) return;
    setState(() => _loading = false);

    if (res.isSuccess) {
      // The reference needed by the validate step may arrive in data or message.
  ...
  Future<void> _validate() async {
    ...
    setState(() => _loading = true);
    final res = await _pinApi.validateChange(ValidatePinRequest(
  ...
  Widget _buildOtpStep() {
    return Column(
      ...
      children: [
        ...
        _button('Confirm Change', _loading, _validate),
      ],
    );
  }
